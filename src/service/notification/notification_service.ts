...
export const generateWorkoutReminder = async (context: AppContext) => {
  var prisma = context.dataSources.prisma;
  var list_of_users = await prisma.baseUser.findMany({
    where: {
      fcm_tokens: {
        some: {},
      },
  ...
  var list_of_token_and_content = await Promise.all(
    list_of_users.map(async function (user) {
      var content_string = "";
      var title_string = "";
      var tokens = user.fcm_tokens;
      var nextWorkout = await prisma.workout.findFirst({
        where: {
          user: {
