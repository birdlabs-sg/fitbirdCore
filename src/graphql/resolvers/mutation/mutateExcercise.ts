...
export const createExcercise = async (_: any, args: any, context: any) => {
  ...
  const targetRegionArray = target_regions.map((target_region_id: any) => ({
    muscle_region_id: target_region_id,
  }));
  const synergistMusclesArray = synergist_muscles.map(
    (synergist_muscles_id: any) => ({ muscle_region_id: synergist_muscles_id })
  );
  const dynamicStabilizerMusclesArray = dynamic_stabilizer_muscles.map(
    (dynamic_stabilizer_muscles_id: any) => ({
      muscle_region_id: dynamic_stabilizer_muscles_id,
    })
  );
  const stabilizerMusclesArray = stabilizer_muscles.map(
    (stabilizer_muscles_id: any) => ({
